...
def init_session_state():
    """Initialize session state variables. Call this at the start of the app."""
    if "executor" not in st.session_state:
        st.session_state.executor = ThreadPoolExecutor(max_workers=30)
...
def get_date_range(period):
    ...
    Returns:
        tuple: (start_date, end_date) where start_date can be None for "Tất cả" option
    """
    end_date = datetime.now().date()
    days_map = {"1 tháng": 30, "3 tháng": 90, "6 tháng": 180, "1 năm": 365, "2 năm": 730, "5 năm": 1825}
    
    if period == "Tất cả":
        # Return None for start_date to indicate all available data
...
def render_main_navigation():
    ...
    with col4:
        if st.button("🤖 AI Chat", key="nav_ai"):
            navigate_to("🤖 AI Chat")
    with col5:
...
# Note: These must be after st.set_page_config, but we initialize executor here

st.set_page_config(page_title="Market Sentiment", layout="wide")

# Initialize session state (must be after st.set_page_config)
...
def clear_content_on_menu_change():
    ...
    keys_to_keep = {"executor", "jobs", "annotations_force_refresh", "main_menu", "ai_chat_history", "ai_chat_display", "ai_chat_messages"}
...
main_menu_options = ["Trang chủ", "Thị trường", "Cổ phiếu", "🤖 AI Chat", "Test"]
...
if main_menu == "Trang chủ":
    ...
    with action_col4:
        if st.button("🤖 AI Chat", key="quick_ai"):
            st.query_params._nav_menu = "🤖 AI Chat"
            st.rerun()
    ...
    with col2:
        ...
        try:
            ...
            if vnindex_data is not None and not vnindex_data.empty:
                ...
                try:
                    # Get more data points to ensure we have at least 2 trading days
                    current_data = get_stock_history('VNINDEX', period='day', count_back=10)
    ...
    if co_phieu_submenu == "-- Chọn --":
        ...
        def display_pb_chart(pb_df, pb_ref, symbol):
            ...
            # Calculate and add Standard Deviation lines
            pb_values = pb_df['pb'].dropna()
            if len(pb_values) > 1:
                pb_mean = pb_values.mean()
                pb_std = pb_values.std()
                
                # Only add StDev lines if we have valid statistics
                if not pd.isna(pb_mean) and not pd.isna(pb_std) and pb_std > 0:
                    std_lines = [
                        (pb_mean + 2 * pb_std, '+2StDv', '#FF0000'),  # Red - very overvalued
                        (pb_mean + 1 * pb_std, '+1StDv', '#FF6B6B'),  # Light red - overvalued
                        (pb_mean - 1 * pb_std, '-1StDv', '#32CD32'),  # Lime green - undervalued
                        (pb_mean - 2 * pb_std, '-2StDv', '#20B2AA'),  # Light Sea Green - very undervalued
                    ]
                    
                    for std_val, label, color in std_lines:
                        fig_pb.add_trace(go.Scatter(
                            x=[pd.Timestamp(pb_df['date'].min()), pd.Timestamp(pb_df['date'].max())],
                            y=[std_val, std_val],
                            mode='lines',
                            name=f"{label}: {std_val:.2f}",
            ...
            if len(pb_values) > 1:
                ...
                if not pd.isna(pb_mean) and not pd.isna(pb_std) and pb_std > 0:
                    plus_2std = pb_mean + 2 * pb_std
                    plus_1std = pb_mean + 1 * pb_std
                    minus_1std = pb_mean - 1 * pb_std
                    minus_2std = pb_mean - 2 * pb_std
                    
                    # Determine valuation based on current P/B vs StDev bands
                    if pb_latest is not None:
                        if pb_latest >= plus_2std:
                            valuation_status = "🔴 Rất đắt"
                            valuation_color = "#FF0000"
                            valuation_desc = f"P/B hiện tại ({pb_latest:.2f}) cao hơn +2StDv ({plus_2std:.2f})"
                        elif pb_latest >= plus_1std:
                            valuation_status = "🟠 Đắt"
                            valuation_color = "#FF6B6B"
                            valuation_desc = f"P/B hiện tại ({pb_latest:.2f}) cao hơn +1StDv ({plus_1std:.2f})"
                        elif pb_latest <= minus_2std:
                            valuation_status = "🟢 Rất rẻ"
                            valuation_color = "#20B2AA"
                            valuation_desc = f"P/B hiện tại ({pb_latest:.2f}) thấp hơn -2StDv ({minus_2std:.2f})"
                        elif pb_latest <= minus_1std:
                            valuation_status = "🔵 Rẻ"
                            valuation_color = "#32CD32"
                            valuation_desc = f"P/B hiện tại ({pb_latest:.2f}) thấp hơn -1StDv ({minus_1std:.2f})"
                        else:
                            valuation_status = "🟡 Bình thường"
                            valuation_color = "#FFD700"
                            valuation_desc = f"P/B hiện tại ({pb_latest:.2f}) nằm trong vùng +/-1StDv"
                        
                        st.markdown(f"""
                        <div style="padding: 15px; border-radius: 10px; background-color: rgba(30,30,30,0.8); margin: 10px 0;">
                            <h4 style="margin: 0; color: {valuation_color};">📊 Đánh giá P/B theo StDev: {valuation_status}</h4>
                            <p style="margin: 5px 0 0 0; color: white;">{valuation_desc}</p>
                            <p style="margin: 5px 0 0 0; color: #888; font-size: 12px;">Mean: {pb_mean:.2f} | StDev: {pb_std:.2f} | +1σ: {plus_1std:.2f} | -1σ: {minus_1std:.2f}</p>
                        </div>
                        """, unsafe_allow_html=True)
        ...
        def display_pe_chart(pe_df, pe_ref, symbol):
            ...
            # Calculate and add Standard Deviation lines
            pe_values = pe_df[pe_col].dropna() if pe_col else pd.Series()
            if len(pe_values) > 1:
                pe_mean = pe_values.mean()
                pe_std = pe_values.std()
                
                # Only add StDev lines if we have valid statistics
                if not pd.isna(pe_mean) and not pd.isna(pe_std) and pe_std > 0:
                    std_lines = [
                        (pe_mean + 2 * pe_std, '+2StDv', '#FF0000'),  # Red - very overvalued
                        (pe_mean + 1 * pe_std, '+1StDv', '#FF6B6B'),  # Light red - overvalued
                        (pe_mean - 1 * pe_std, '-1StDv', '#32CD32'),  # Lime green - undervalued
                        (pe_mean - 2 * pe_std, '-2StDv', '#20B2AA'),  # Light Sea Green - very undervalued
                    ]
                ...
                fig_pe.update_layout(
                    ...
                    xaxis_title=dict(text='<b>Date</b>', font=dict(color='white')),
                    yaxis=dict(
                        title=dict(text='<b>P/E</b>', font=dict(color='white')),
                        showgrid=True, gridcolor='rgba(255,255,255,0.1)',
                        tickfont=dict(color='white'),
                        title_font=dict(color='white'),
                        autorange=True
            ...
            if len(pe_values) > 1:
                ...
                if not pd.isna(pe_mean) and not pd.isna(pe_std) and pe_std > 0:
                    plus_2std = pe_mean + 2 * pe_std
                    plus_1std = pe_mean + 1 * pe_std
                    minus_1std = pe_mean - 1 * pe_std
                    minus_2std = pe_mean - 2 * pe_std
                    
                    # Determine valuation based on current P/E vs StDev bands
                    if pe_latest is not None:
                        if pe_latest >= plus_2std:
                            valuation_status = "🔴 Rất đắt"
                            valuation_color = "#FF0000"
                            valuation_desc = f"P/E hiện tại ({pe_latest:.2f}) cao hơn +2StDv ({plus_2std:.2f})"
                        elif pe_latest >= plus_1std:
                            valuation_status = "🟠 Đắt"
                            valuation_color = "#FF6B6B"
                            valuation_desc = f"P/E hiện tại ({pe_latest:.2f}) cao hơn +1StDv ({plus_1std:.2f})"
                        elif pe_latest <= minus_2std:
                            valuation_status = "🟢 Rất rẻ"
                            valuation_color = "#20B2AA"
                            valuation_desc = f"P/E hiện tại ({pe_latest:.2f}) thấp hơn -2StDv ({minus_2std:.2f})"
                        elif pe_latest <= minus_1std:
                            valuation_status = "🔵 Rẻ"
                            valuation_color = "#32CD32"
                            valuation_desc = f"P/E hiện tại ({pe_latest:.2f}) thấp hơn -1StDv ({minus_1std:.2f})"
                        else:
                            valuation_status = "🟡 Bình thường"
                            valuation_color = "#FFD700"
                            valuation_desc = f"P/E hiện tại ({pe_latest:.2f}) nằm trong vùng +/-1StDv"
                        
                        st.markdown(f"""
                        <div style="padding: 15px; border-radius: 10px; background-color: rgba(30,30,30,0.8); margin: 10px 0;">
                            <h4 style="margin: 0; color: {valuation_color};">📊 Đánh giá P/E theo StDev: {valuation_status}</h4>
                            <p style="margin: 5px 0 0 0; color: white;">{valuation_desc}</p>
                            <p style="margin: 5px 0 0 0; color: #888; font-size: 12px;">Mean: {pe_mean:.2f} | StDev: {pe_std:.2f} | +1σ: {plus_1std:.2f} | -1σ: {minus_1std:.2f}</p>
                        </div>
                        """, unsafe_allow_html=True)
        ...
        with tab_pb:
            ...
            with col2:
                pb_period = st.selectbox(
                    "Chọn khoảng thời gian",
                    ["1 tháng", "3 tháng", "6 tháng", "1 năm", "2 năm", "5 năm", "Tất cả", "Tùy chỉnh"],
                ...
                if not symbol_pb:
                    ...
                    with st.spinner(f"Đang tải dữ liệu P/B cho {symbol_pb}..."):
                        try:
                            ...
                            elif pb_period == "Tất cả":
                                # For "Tất cả", use a very old date to get all available data (10 years)
                    ...
                    with st.spinner(f"Đang tải dữ liệu giá cho {symbol_price}..."):
                        try:
                            ...
                            if price_period == "Tùy chỉnh":
                                ...
                                count_back = 365
                            elif price_period == "Tất cả":
                                # For "Tất cả", use a very large count_back to get all available data
                                start_date = datetime.now().date() - timedelta(days=3650)
                                end_date = datetime.now().date()
                                count_back = 3650
                            else:
                            ...
                            price_df = get_stock_history(symbol_price, period="day", end_date=end_date.strftime('%Y-%m-%d'), count_back=count_back)
                ...
                if stock_inv_status == "running":
                    ...
                    try:
                        ...
                        col_names_vi = {
                            ...
                            'foreignIndividualNetValue': 'Cá nhân nước ngoài',
                            'foreignInstitutionalNetValue': 'Tổ chức nước ngoài'
                        }
