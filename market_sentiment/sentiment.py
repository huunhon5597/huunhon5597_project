...
# Cookies are read directly from file
# Priority: local file sstock_cookie.txt
...
def sentiment (start_date, end_date=None):
    ...
    # Get cookies - đọc trực tiếp từ file
    cookies = None
    
    # Try multiple possible locations for the cookie file
    possible_paths = [
        "sstock_cookie.txt",
    ...
    for cookie_path in possible_paths:
        try:
            if os.path.exists(cookie_path):
                ...
                print(f"Got cookies from {cookie_path}")
                break
        except Exception as e:
            print(f"Warning: Could not read cookies from {cookie_path}: {e}")
    
    if not cookies:
        print(f"Warning: Cookie file not found in any of: {possible_paths}")
    
    # If cookies not available, return empty DataFrame
    if not cookies:
        print("Warning: No cookies available for sstock API - returning empty DataFrame")
        return pd.DataFrame()
...
def volatility(symbol='VNINDEX', end_date=None, countback=252, return_summary=False, forecast_days=10):
    ...
    try:
        ...
        if not ARCH_AVAILABLE:
            print("Warning: arch library not available, using simple volatility")
...
def high_low_index(start_date, end_date=None):
    ...
    hl_df['hl_index'] = ta.sma(hl_df['record_high_percent'].fillna(0), length=10) if TA_AVAILABLE else hl_df['record_high_percent'].fillna(0).rolling(window=10).mean()
...
def bpi(start_date, end_date=None):
    ...
    def process_stock_for_bpi(symbol, start_date, end_date):
        ...
        try:
            ...
            df['ema20'] = ta.ema(df['close'], length=20) if TA_AVAILABLE else df['close'].ewm(span=20, adjust=False).mean()
            df['ema50'] = ta.ema(df['close'], length=50) if TA_AVAILABLE else df['close'].ewm(span=50, adjust=False).mean()
...
def ma(start_date, end_date=None):
    ...
    ma_df['ma50'] = ta.sma(ma_df['close'], length=50) if TA_AVAILABLE else ma_df['close'].rolling(window=50).mean()
    ma_df['ma200'] = ta.sma(ma_df['close'], length=200) if TA_AVAILABLE else ma_df['close'].rolling(window=200).mean()
